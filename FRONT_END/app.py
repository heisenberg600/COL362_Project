...
class User():
    ...
    def setData(self, username, level=2):
        if(level == 2):
            uData = query.select(['*'], ['persons'],
                                 f"username = '{username}'")[0]
            uData = {query.cols[i]: uData[i] for i in range(len(uData))}
        elif(level == 1):
            uData = query.select(['*'], ['managers'],
                                 f"username = '{username}'")[0]
            uData = {query.cols[i]: uData[i] for i in range(len(uData))}
            rData = query.select(['*'], ['managersView'],
                                 f"id = '{uData['id']}'")[0]
...
def addUser(username, password, firstname, lastname, gender, dob, level):
    if (available_username(username)):
        ...
        query.insert('persons(id,firstname,lastname,gender,phone,email,username,password,dob,priviledge)', [
                     f"'{cur_id}'", f"'{firstname}'", f"'{lastname}'", f"'{gender}'", "'NULL'", "'NULL'", f"'{username}'", f"'{password}'", f"'{dob}'", "2"])
        return (0, '')
    else:
...
def check_username_password(username, password, table='persons'):
    cur_rows = query.select(
        ['username'], [table], f"username= '{username}' and password = '{password}'")
    if(len(cur_rows) == 0):
        return False
    return True
...
def signup():
    if(request.method == 'POST'):
        ...
        current_details = addUser(
            username, password, firstname, lastname, gender, dob, 0)
...
def update():
    if(user.login):
        ...
        pages = ['update_admin.html',
                 'update_manager.html', 'update_user.html']
...
def changeRest(site, title, data):
    ...
    if(data.get('restSelected') != None and data.get('restSelected') != ""):
        ...
        if(mode == 'byresid'):
            ...
            ans = query.select(['restId'], ['restaurants'],
                               f"phoneNo = '{data.get('phone')}'")
            ...
            ans = query.select(['restId'], [
                               'restaurants'], f"latitude = '{data.get('lat')}' and longitude = '{data.get('long')}'")
            if(len(ans) > 0 and len(ans[0]) > 0):
                rest_id = ans[0][0]
            else:
    ...
    if(user.login):
        if(request.method == 'POST'):
            ...
            if(data.get('restS') != None and data.get('restS') != ''):
                ...
                try:
                    query.insert(
                        'reviews', [f'{user.data["id"]}', f'{rest_id}', f'{stars}'])
                    flash("Review added")
                except:
        ...
        if(request.method == 'POST'):
            ...
            if(changed == None):
                ...
                if(data.get('restS') != None and data.get('restS') != ''):
                    try:
                        rest_id = data.get('restS')
                        ans = query.select(['latitude', 'longitude', 'name'], [
                                           'restaurants'], f'restId = {rest_id}')[0]
    ...
    if(s != None or (data.get('oldpass') != None and data.get('oldpass') == user.data['password'])):
        if(data.get('newpass') == data.get('confirmpass')):
            try:
                ...
                query.update(tables[user.level()], {'password': data.get(
                    'newpass')}, f'id = {user.data["id"]}')
...
def checkPass(data):
    if(data.get('firstname') != None):
        ...
        dob = data.get('dob')
        ans = query.select(['id'], [
                           'persons'], f"firstname = '{firstame}' and lastname = '{lastname}' and dob = '{dob}'")
        if(len(ans) > 0 and len(ans[0]) > 0):
